...
class IPAdapterUnifiedLoader:
    ...
    def load_models(self, model, preset, lora_strength=0.0, provider="CPU", ipadapter=None):
        ...
        # 使用更可靠的方法判断是否为SDXL：检查model_config而不是model类型
        try:
            import comfy.supported_models
            is_sdxl = isinstance(model.model.model_config, (comfy.supported_models.SDXL, comfy.supported_models.SDXLRefiner))
        except (AttributeError, ImportError):
            # 回退到原来的方法
            is_sdxl = isinstance(model.model, (comfy.model_base.SDXL, comfy.model_base.SDXLRefiner, comfy.model_base.SDXL_instructpix2pix))
        
        # 如果预设是 FACEID PLUS V2 且找不到 SD15 模型，强制使用 SDXL
        # 因为 FACEID PLUS V2 通常用于 SDXL，且用户可能只有 SDXL 模型文件
        ipadapter_file, is_insightface, lora_pattern = get_ipadapter_file(preset, is_sdxl)
        if ipadapter_file is None and preset.lower().startswith("faceid plus v2") and not is_sdxl:
            # 尝试使用 SDXL 模型
            print(f"WARNING: SD15 model not found for {preset}, trying SDXL instead...")
            is_sdxl = True
            ipadapter_file, is_insightface, lora_pattern = get_ipadapter_file(preset, is_sdxl)
        
        # 调试信息：如果还是找不到文件，输出判断结果
        if ipadapter_file is None:
            print(f"DEBUG: is_sdxl={is_sdxl}, preset={preset}, model_type={type(model.model)}, model_config_type={type(model.model.model_config) if hasattr(model.model, 'model_config') else 'N/A'}")
        if ipadapter_file is None:
