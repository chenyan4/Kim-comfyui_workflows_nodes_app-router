...
def get_ipadapter_file(preset, is_sdxl):
    ...
    # 调试信息
    if not ipadapter_list:
        print(f"WARNING: ipadapter_list is empty. Checking folder_paths...")
        if "ipadapter" in folder_paths.folder_names_and_paths:
            paths, _ = folder_paths.folder_names_and_paths["ipadapter"]
            print(f"ipadapter paths: {paths}")
            import os
            for path in paths:
    ...
    if not ipadapter_file:
        print(f"ERROR: No file matching pattern '{pattern}' found in ipadapter_list")
        print(f"Available files: {ipadapter_list}")
        print(f"Preset: {preset}, is_sdxl: {is_sdxl}")
