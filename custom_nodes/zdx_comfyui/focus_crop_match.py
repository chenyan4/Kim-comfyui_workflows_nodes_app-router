...
"""
    ...
    def crop_by_mask_v2(self, image, all_mask=None, up_keep=0.2, down_keep=0.2, right_keep=0.2, left_keep=0.2,  is_focus=True, mask=None, crop_ratio="original"):
        ...
        if all_mask is not None and len(all_mask.shape) > 3:
            all_mask = all_mask[0]

        NEED_CROP_TWICE = False
        # all_mask = copy.deepcopy(mask)
        if all_mask is None:
            all_mask = mask
        else:
            ...
            if len(mask.shape) > 3:
                mask = mask[0]
            all_mask = self.zdxmaskadd.subtract_masks(
                masks_a=mask,
                masks_b=all_mask,
            )[0]

        all_mask_pil = tensor2pil(all_mask).convert("L")
        img_h, img_w = image.shape[1], image.shape[2]
        bbox = all_mask_pil.getbbox()
        if bbox is None:
            return (image, all_mask, None, None, all_mask, crop_h, crop_w, None)
        x, y, x_w, y_h = bbox
        crop_w, crop_h = x_w - x, y_h - y
        # expected expand pixe
        top_pix = int(crop_h * up_keep)
        down_pix = int(crop_h * down_keep)
        right_pix = int(crop_h * right_keep)
        left_pix = int(crop_h * left_keep)
        # Limit expansion to image boundaries

        total_w, total_h = crop_w + left_pix + right_pix, crop_h + top_pix + down_pix
        # 确保total_w, total_h  整除8
        total_w, total_h = total_w - total_w % 8, total_h - total_h % 8   # 向下取整
        # total_w, total_h = min(total_w, img_w), min(total_h, img_h)
        if crop_ratio != "original":
            target_ratio = ratio_map.get(crop_ratio)
            
            if target_ratio:
                curr_w = crop_w + left_pix + right_pix
                curr_h = crop_h + top_pix + down_pix
                curr_ratio = curr_w / curr_h
                
                if curr_ratio > target_ratio:
                    # Too wide, increase height
                    needed_h = curr_w / target_ratio
                    diff_h = int(needed_h - curr_h)
                    extra_pix = 0 if diff_h % 2 == 0 else 1
                    top_pix += diff_h // 2
                    top_pix += extra_pix
                    down_pix += diff_h - (diff_h // 2)
                else:
                    # Too tall, increase width
                    needed_w = curr_h * target_ratio
                    diff_w = int(needed_w - curr_w)
                    extra_pix = 0 if diff_w % 2 == 0 else 1
                    left_pix += diff_w // 2
                    left_pix += extra_pix
                    right_pix += diff_w - (diff_w // 2)

                # Fix: Check bounds and shift/ padding if necessary
                total_w = crop_w + left_pix + right_pix
                total_h = crop_h + top_pix + down_pix
                print(f"total_w, total_h: {total_w}, {total_h} is eqal?")

                # 1. Shift if position exceeds bounds
                # Left
                if x - left_pix < 0:
                    shift = -(x - left_pix)
                    left_pix -= shift
                    right_pix += shift
                # Right
                if (x + crop_w) + right_pix > img_w:
                    shift = ((x + crop_w) + right_pix) - img_w
                    right_pix -= shift
                    left_pix += shift
                # Top
                if y - top_pix < 0:
                    ...
                    down_pix += shift
                # Bottom
                if (y + crop_h) + down_pix > img_h:
                    shift = ((y + crop_h) + down_pix) - img_h
                    down_pix -= shift
                    top_pix += shift
        ...
        image_mask_cropbymask_1 = self.layerutility_cropbymask_v2.crop_by_mask_v2(
            ...
            detect="mask_area",
            top_reserve=top_pix,
            bottom_reserve=down_pix,
            left_reserve=left_pix,
            right_reserve=right_pix,
            round_to_multiple="8",
            image=image,
            mask=all_mask,
        )
        # 2. Padding if size not enough
        image_mask_pad_1 = (image_mask_cropbymask_1[0], image_mask_cropbymask_1[1], )
        pad_left, pad_right, pad_top, pad_bottom = 0, 0, 0, 0
        if crop_ratio != "original":
            _, crop_h, crop_w, _ = image_mask_cropbymask_1[0].shape
            # Recalculate target dimensions to account for potential rounding in crop_by_mask_v2
            total_w = max(total_w, crop_w)
            total_h = max(total_h, crop_h)
            target_ratio = ratio_map.get(crop_ratio)
            if target_ratio:
                 curr_ratio = total_w / total_h
                 if curr_ratio > target_ratio:
                     total_h = int(total_w / target_ratio)
                 elif curr_ratio < target_ratio:
                     total_w = int(total_h * target_ratio)
            need_w, need_h = total_w - crop_w, total_h - crop_h
            
            extra_pix_w = need_w % 2
            extra_pix_h = need_h % 2
            
            pad_left = max(0, need_w // 2 + extra_pix_w)
            pad_right = max(0, need_w // 2) # Right gets the smaller half
            pad_top = max(0, need_h // 2 + extra_pix_h)
            pad_bottom = max(0, need_h // 2) # Bottom gets the smaller half
            ...
            print(f"image_mask_pad_1 shape: {image_mask_pad_1[0].shape}")
        ...
        layerutility_cropboxresolve_491 = (
            self.layerutility_cropboxresolve.crop_box_resolve(
                crop_box=get_value_at_index(image_mask_cropbymask_1, 2)
            )
        )
        bbox = (layerutility_cropboxresolve_491[0], layerutility_cropboxresolve_491[1], layerutility_cropboxresolve_491[2], layerutility_cropboxresolve_491[3])
        bbox = [int(b) for b in bbox]
        bbox = [bbox[0], bbox[1], bbox[2]+bbox[0], bbox[3]+bbox[1]]
        # Store full padding info: [left, top, right, bottom]
        info = {"bbox": bbox, "pad": [pad_left, pad_top, pad_right, pad_bottom], "crop_box": image_mask_cropbymask_1[2]}
        print(info)
        return (
            image_mask_pad_1[0], 
            image_mask_pad_1[1], 
