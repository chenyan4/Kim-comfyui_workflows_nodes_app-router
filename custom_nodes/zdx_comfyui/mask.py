...
class MaskToPointsAndBBox:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "mask": ("MASK",),
                "is_max_include": ("BOOLEAN", {"default": True}),
                "grid_spacing": ("INT", {"default": 32, "min": 1, "max": 512, "step": 1}),
            },
            "optional": {
                "image": ("IMAGE",),
            }
    ...
    RETURN_TYPES = ("STRING", "STRING", "BBOX", "IMAGE")
    RETURN_NAMES = ("positive_coords", "negative_coords", "bbox", "debug_image")
    FUNCTION = "get_points_and_bbox"
    CATEGORY = "zdx/mask"

    def get_points_and_bbox(self, mask, is_max_include, grid_spacing=32, image=None):
        if len(mask.shape) == 3:
        ...
        if len(y_idxs) == 0:
            return (json.dumps([]), json.dumps([]), [], image if image is not None else torch.zeros((1, h, w, 3)))

        if is_max_include:
            x1, y1 = np.min(x_idxs), np.min(y_idxs)
            x2, y2 = np.max(x_idxs), np.max(y_idxs)
            bboxes.append((int(x1), int(y1), int(x2+1), int(y2+1)))
        else:
        ...
        def adaptive_sample(indices_y, indices_x, max_points=10):
            if len(indices_y) == 0: return []
            
            min_y, max_y = np.min(indices_y), np.max(indices_y)
            min_x, max_x = np.min(indices_x), np.max(indices_x)
            
            range_y = max_y - min_y
            range_x = max_x - min_x
            
            # Use max(1, ...) to avoid division by zero or step 0
            step_x = max(1, range_x / max_points)
            step_y = max(1, range_y / max_points)
            
            # Use mgrid based on the bounding box of the points
            gy, gx = np.mgrid[min_y:max_y+1:step_y, min_x:max_x+1:step_x]
            gy = gy.flatten().astype(int)
            gx = gx.flatten().astype(int)
            
            # Keep points that are inside the valid area
            lookup = np.zeros((h, w), dtype=bool)
            lookup[indices_y, indices_x] = True
            
            valid_mask_g = (gy < h) & (gx < w)
            gy = gy[valid_mask_g]
            gx = gx[valid_mask_g]
            
            is_valid = lookup[gy, gx]
            return list(zip(gx[is_valid], gy[is_valid]))

        # (A) Positive Points: Erode slightly to avoid boundary overlaps
        # Erode the mask by a small amount to pull points away from the edge
        erode_kernel_size = 3
        ...
        # If erosion kills small features, fallback to original mask for those parts
        # or just use original mask if eroded is empty
        if cv2.countNonZero(eroded_mask) == 0:
            target_pos_mask = binary_mask
        else:
            target_pos_mask = eroded_mask

        py, px = np.where(target_pos_mask > 0)
        sampled_pos = adaptive_sample(py, px, max_points=10)
        for x, y in sampled_pos:
            pos_points.append({"x": int(x), "y": int(y)})

        # (B) Negative Points: Outer boundary sampling
        # Increase dilation kernel to get more negative points
        # And ensure we sample enough by increasing max_points for negative or ensuring the ring is thick enough
        
        # 1. Create a "Safe Zone" gap between positive and negative
        # Dilate the original mask slightly (e.g. 10px) - this area is forbidden for negative points
        # to prevent overlap with positive boundary.
        gap_kernel_size = 15
        gap_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (gap_kernel_size, gap_kernel_size))
        forbidden_mask = cv2.dilate(binary_mask, gap_kernel, iterations=1)
        
        # 2. Define the Negative Sampling Ring
        # Dilate further to find the area we WANT to sample from.
        # Make this ring thicker to allow more points.
        outer_kernel_size = 60 # Increased to get a much thicker band
        outer_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (outer_kernel_size, outer_kernel_size))
        outer_mask = cv2.dilate(binary_mask, outer_kernel, iterations=1)
        
        # Negative region = Outer Ring - Forbidden Gap
        # Also ensure we are strictly in background of original image (binary_mask == 0)
        # (Though forbidden_mask covers binary_mask, so forbidden_mask == 0 implies binary_mask == 0)
        neg_mask_region = (outer_mask > 0) & (forbidden_mask == 0)
        ny, nx = np.where(neg_mask_region)
        
        # Increase max_points for negative to get more density around the object
        sampled_neg = adaptive_sample(ny, nx, max_points=20) 
        for x, y in sampled_neg:
            neg_points.append({"x": int(x), "y": int(y)})
            
        # Deduplicate
        unique_pos = { (p['x'], p['y']) for p in pos_points }
        pos_points_final = [ {'x': x, 'y': y} for x, y in unique_pos ]
        
        unique_neg = { (p['x'], p['y']) for p in neg_points }
        neg_points_final = [ {'x': x, 'y': y} for x, y in unique_neg ]
        ...
        if image is not None:
            ...
            for p in pos_points_final:
                cv2.circle(img_np, (p['x'], p['y']), 15, (0, 255, 0), -1) 
            
            # Draw negative points (Red)
            for p in neg_points:
                cv2.circle(img_np, (p['x'], p['y']), 15, (255, 0, 0), -1) 
            
            # Draw BBox (Blue)
            for bbox in bboxes:
                 cv2.rectangle(img_np, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 0, 255), 2)

            debug_image = torch.from_numpy(img_np.astype(np.float32) / 255.0).unsqueeze(0)
        else:
            debug_image = torch.zeros((1, h, w, 3))

        return (json.dumps(pos_points_final), json.dumps(neg_points), bboxes, debug_image)

# image_concat_mask, image_concat_mask_v1, EmptyImagePro, MaskAreaReColor
_NODE_CLASS_MAPPINGS = {
    "MaskToPointsAndBBox": MaskToPointsAndBBox,
...
_NODE_DISPLAY_NAME_MAPPINGS = {
    "MaskToPointsAndBBox": "Mask To Points & BBox",
    "GrowMaskEdgeBlur": "GrowMaskEdgeBlur", 
