...
def reshape_mask(input_mask, output_shape,video_inpainting=False):
    ...
    # Handle 5D case: (batch, channels, frames, height, width) — use when shape is 5D or video_inpainting
    # Some models (e.g. FLUX) use 5D latent with frames=1; must use 3 spatial dims for interpolate.
    if len(output_shape) == 5 or video_inpainting:
        target_frames = output_shape[2]
        target_height, target_width = output_shape[3], output_shape[4]
        if input_mask.ndim == 4:
            print('Video case - input_mask initial shape (4D):', input_mask.shape)
        ...
        # input_mask is 5D: (batch, channels, frames, height, width)
    ...
    else:  # 2D image case: 4D (N, C, H, W) or 3D (N, H, W)
        # F.interpolate expects 4D (N, C, H, W) for size=(H, W). If mask is 3D (N, H, W), add channel dim.
        if input_mask.ndim == 3:
            input_mask = input_mask.unsqueeze(1)  # (N, H, W) -> (N, 1, H, W)
        if comfyui_version.__version__ < "0.6.0":
